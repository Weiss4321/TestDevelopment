...
public class B0290 extends Batch{
    public static String cvsident = "@(#) $Header: /var/cvsroot/java/src/hr/vestigo/modules/collateral/batch/bo29/B0290.java,v 1.2 2008/09/10 12:01:42 hrazst Exp $";
    ...
    private void startBatch() throws Exception{
        ...
        while (iterFinaVehicle.next()){
            ...
            if(!iterFinaVehicle.upisano_pravo().equals("DA")){
                ...
                pomVector=checkForErrors(iterFinaVehicle.br_sasije());
        ...
        String fileUpdatedData=writeFile(iterReportData, bc.getOutDir(), "preuzetaVozila_"+fileDateTime+".cvs");
        
        iterReportData=bo291.fetchDataForReport(colProId, false); 
        fileDateTime=DateUtils.createYYYYMMDDString(DateUtils.getCurrentDate())+ "-" +TimeUtils.getCurrentTime().toString().replace(":", ".");        
        String fileNotUpdatedData=writeFile(iterReportData, bc.getOutDir(), "nepreuzetaVozila_"+fileDateTime+".cvs");
    ...
    private Vector checkForErrors(String brSasije) throws Exception{
        ...
        laAccId=bo291.getLaAccId(collHfPriorId);
        if(laAccId==null){
            returnVector.add("5");
            return returnVector;
        }else if(laAccId.compareTo(new BigDecimal("-1)"))==0){
            returnVector.add("4");
            return returnVector;
        }  
