...
import java.util.*;
//COLL_LOANSTOCK
//ZADUZNICA QBE SELECT 17.08.2006
/**
* CO252 class selects all data from table COLL_LOANSTOCK, COLL_HEAD
 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class CO252 extends JDBCScrollableRemoteTransaction {

	public static String cvsident = "@(#) $Header: /var/cvsroot/java/src/hr/vestigo/modules/collateral/jcics/co25/CO252.java,v 1.4 2006/09/21 10:20:38 hrasia Exp $";;
	
	public DeclCO25 decl = null;
	private ResultSet rs = null;
	int i = 0;
	public CO252 (DeclCO25 decl) {
		this.decl = decl;
	}
	
	
	public ResultSet executeScrollable(TransactionContext tc) throws Exception {
		String screen_entry_param = decl.loanstockqbemapp.ScreenEntryParam;
		String phase="";
		String proc_status="";
		...
		setLevel(decl.loanstockqbemapp.ActionListLevel);
		setFetchSize(tc.ACTION_LIST_FETCH_SIZE);
		Connection con = tc.getConnection();
		PreparedStatement prStmt = null;
		
		
		 tc.info("decl.loanstockqbemapp.RealEstate_txtOwnerQbeRegNo2		" + decl.loanstockqbemapp.RealEstate_txtOwnerQbeRegNo2  );             											
		 tc.info("decl.loanstockqbemapp.RealEstate_txtCarrierQbeRegNo2	    " + decl.loanstockqbemapp.RealEstate_txtCarrierQbeRegNo2 );    
		...
		String sql = " SELECT distinct " +
					  ...
					  " INNER JOIN coll_loanstock  colloan ON  ch.col_hea_id = colloan.col_hea_id " +
			...
			String p_owner="";
			if(decl.loanstockqbemapp.RealEstate_txtOwnerQbeRegNo2 != null && !"".equals(decl.loanstockqbemapp.RealEstate_txtOwnerQbeRegNo2)){
				//Vlasnik zaduznice je duznik payee_reg_no
				p_owner = decl.loanstockqbemapp.RealEstate_txtOwnerQbeRegNo2;
				p_owner = p_owner.trim();
				valuesW.add(p_owner);	
				sql = sql + " AND colloan.payee_reg_no = ?  ";
			}
				
			
			String p_carrier="";
			if(decl.loanstockqbemapp.RealEstate_txtCarrierQbeRegNo2 != null && !"".equals(decl.loanstockqbemapp.RealEstate_txtCarrierQbeRegNo2)){

				p_carrier = decl.loanstockqbemapp.RealEstate_txtCarrierQbeRegNo2;
			...
			String accNo= "";
			if(decl.loanstockqbemapp.RealEstate_txtQbeAccNo2 != null && !"".equals(decl.loanstockqbemapp.RealEstate_txtQbeAccNo2)){
				
				
				if(decl.loanstockqbemapp.RealEstate_txtQbeAccNo2.indexOf("*") != -1){
					sql = sql + " AND lobe.acc_no like ? ";
				}else{
					sql = sql + " AND lobe.acc_no = ? ";
				}
				accNo = decl.loanstockqbemapp.RealEstate_txtQbeAccNo2;
				valuesW.add(accNo);	
				
			}
			
			String requestNo= "";
			if(decl.loanstockqbemapp.RealEstate_txtQbeRequestNo2 != null && !"".equals(decl.loanstockqbemapp.RealEstate_txtQbeRequestNo2)){
				
				
				if(decl.loanstockqbemapp.RealEstate_txtQbeRequestNo2.indexOf("*") != -1){
					sql = sql + " AND lobe.request_no like ? ";
				}else{
					sql = sql + " AND lobe.request_no = ? ";
				}
				requestNo = decl.loanstockqbemapp.RealEstate_txtQbeRequestNo2;
				valuesW.add(requestNo);	
				
			}
	 	 
			String sifra= "";
			if(decl.loanstockqbemapp.RealEstate_txtQbeCode2 != null && !"".equals(decl.loanstockqbemapp.RealEstate_txtQbeCode2)){
				
				
				if(decl.loanstockqbemapp.RealEstate_txtQbeCode2.indexOf("*") != -1){
					sql = sql + " AND ch.col_num like ? ";
				}else{
					sql = sql + " AND ch.col_num = ? ";
				}
				sifra = decl.loanstockqbemapp.RealEstate_txtQbeCode2;
				valuesW.add(sifra);	
			...
			if(proc_status!=null) {
				...
				if ( proc_status.trim().equals("0") || proc_status.trim().equals("5")) {
					...
					valuesW.add(decl.loanstockqbemapp.use_id);	
					
					sql = sql + " AND ch.org_uni_id = ? ";
					valuesW.add(decl.loanstockqbemapp.org_uni_id);	
				...
				if(proc_status.trim().equals("3")){
					if(decl.loanstockqbemapp.RealEstate_Qbe_ORG_UNI_ID2 != null ){
						sql = sql + " AND ch.origin_org_uni_id = ? ";
						valuesW.add(decl.loanstockqbemapp.RealEstate_Qbe_ORG_UNI_ID2);	
					}
					if(decl.loanstockqbemapp.RealEstate_QbeUSE_OPEN_ID2 != null ){
						sql = sql + " AND ch.use_open_id = ? ";
						valuesW.add(decl.loanstockqbemapp.RealEstate_QbeUSE_OPEN_ID2);	
					}
				}
			...
			if(decl.loanstockqbemapp.RealEstate_txtQbeDateFrom2 != null && decl.loanstockqbemapp.RealEstate_txtQbeDateUntil2 != null ){
				
				sql = sql + " AND date(ch.opening_ts) between  ? AND ? ";
				valuesW.add(decl.loanstockqbemapp.RealEstate_txtQbeDateFrom2);	
				valuesW.add(decl.loanstockqbemapp.RealEstate_txtQbeDateUntil2);	
				
			}else{
				if(decl.loanstockqbemapp.RealEstate_txtQbeDateFrom2 != null ){
			...
			tc.info("decl.loanstockqbemapp.RealEstate_QbeUSE_OPEN_ID2 " + decl.loanstockqbemapp.RealEstate_QbeUSE_OPEN_ID2);
			tc.info("decl.loanstockqbemapp.RealEstate_Qbe_ORG_UNI_ID2 " + decl.loanstockqbemapp.RealEstate_Qbe_ORG_UNI_ID2);
			tc.info("decl.loanstockqbemapp.RealEstate_QbeDateFrom2 " + decl.loanstockqbemapp.RealEstate_txtQbeDateFrom2);
			tc.info("decl.loanstockqbemapp.RealEstate_QbeDateUntil2 " + decl.loanstockqbemapp.RealEstate_txtQbeDateUntil2);
			tc.info("\n ");
			tc.debug(sql);
			tc.info("\n ");
...
//			
		...
		if(proc_status!=null) {
			...
			if (proc_status.trim().equals("0")) {
				//referentska lista za unos, po referentu  			  
		  		...
		  		tc.info("tu sam .... proc_status : "+decl.loanstockqbemapp.use_id); 			
		  		tc.info("tu sam .... proc_status : "+decl.loanstockqbemapp.org_uni_id); 			
	 ...
	 public void populateRowData() throws Exception {
		...
		decl.loanstockqbemapp.tblColWorkList.addRow(row, unique);
